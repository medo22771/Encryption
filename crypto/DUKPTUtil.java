...
public class DUKPTUtil {
    ...
    //private static final byte[] BDK_MASK = 		 StringUtil.hexStringToBytes("37233E890B0104E9BC943D0E45EAE5A7");
    ...
    public static byte[] calculateKsnEncryptedKey(byte[] ksn) {
        //BigInteger cryptedKey = (new BigInteger(ksn)).and(new BigInteger(KSN_ENC_KEY_MASK));
        byte[] maskedKsn = ByteArrayUtil.and(ksn, KSN_ENC_KEY_MASK);
        byte[] cryptedKey = ByteArrayUtil.subArray(maskedKsn, (ksn.length / 2), maskedKsn.length - 1);
        return cryptedKey;
    }
